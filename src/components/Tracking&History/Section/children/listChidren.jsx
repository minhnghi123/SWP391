...
export default function ListChildren({ id }) {
  ...
  const handleCreateChild = async (e) => {
    setIsLoading(true);
    e.preventDefault();
    if (![0, 1].includes(Number(currentChild.gender)))
      return setErr("Please choose a valid gender.");
  ...
  const handleUpdateChild = async (e) => {
    ...
    if (
      listChildren.find(
        (child) =>
          ...
          child.status.toLowerCase() === "tracking"
      )
    ) {
      toast.error(
        "This child is currently being tracked. Please complete the tracking process before updating."
      );
      setIsLoading(false);
      setIsModalOpen(false);
      return;
    ...
    try {
      const res = await api.put(
        `${url}/Child/update-child/${currentChild.id}`,
        currentChild
      );
      if (res.status === 200) {
  ...
  const handleDeleteChild = async () => {
    ...
    if (
      listChildren.find(
        (child) =>
          child.id === deleteId && child.status.toLowerCase() === "tracking"
      )
    ) {
      toast.error(
        "This child is currently being tracked. Please complete the tracking process before deleting."
      );
  ...
  const handleFilter = (status) => {
    ...
    if (status === "all") {
      ...
      setFilteredChildren(
        listChildren.filter(
          (child) => Number(child.gender) === (status === "male" ? 0 : 1)
        )
      );
    }
  };
