import { Calendar, Clock, CheckCircle, ChevronDown, ChevronUp, AlertCircle, Shield, User, PlusCircle, Filter, XCircle, History, AudioLines, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function VaccineSchedules({ sortLinkList, setTrigger }) {
  ...
  const sortOptions = [
    ...
    { id: 'date_desc', label: 'Date (Newest First)', icon: <ArrowDown className="w-4 h-4" /> },
    { id: 'name_asc', label: 'Name (A-Z)', icon: <ArrowUp className="w-4 h-4" /> },
    { id: 'name_desc', label: 'Name (Z-A)', icon: <ArrowDown className="w-4 h-4" /> },
  ...
  if (historyTracking) {
    return (
      <HistoryTracking sortLinkList={sortLinkList} setTrigger={setTrigger} setHistoryTracking={setHistoryTracking} historyTracking={historyTracking} />
    )
  }
  return (
        ...
        (
          ...
          <div className="space-y-6">
            {sortedLinkList.map((chain, index) => {
              ...
              const completed = chain.filter((item) => item.status.toLowerCase() === 'success').length;
                ...
                >
                  <div className="flex flex-row-reverse items-center gap-4 mb-2">
                    ...
                    <div className="flex-1 bg-white p-5 rounded-xl border border-gray-100 hover:border-indigo-200 transition-all duration-300 hover:shadow-md">
                      <ProgressBar vaccineName={vaccineName} current={completed} total={total} percentage={percentage} doses={chain} />
                    </div>
                  </div>
                            ...
                            >
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    <h4 className="font-semibold text-gray-800 text-lg">Dose {doseIndex + 1}</h4>
                                    <span className="text-sm text-gray-500">{dose.vaccineName || 'Unknown Vaccine'}</span>
                                  </div>
                                </div>
