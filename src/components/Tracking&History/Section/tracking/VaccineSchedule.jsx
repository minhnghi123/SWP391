import { Calendar, Clock, CheckCircle, ChevronDown, ChevronUp, AlertCircle, Shield, User, PlusCircle, Filter, Bookmark, XCircle } from 'lucide-react';
...
export default function VaccineSchedules({ sortLinkList, setTrigger, handleSortChange }) {
  ...
  const sortedLinkList = [...sortLinkList].sort((a, b) => {
    const aHasCancel = a.some(item => item.status.toLowerCase() === 'cancel');
    const bHasCancel = b.some(item => item.status.toLowerCase() === 'cancel');
    if (aHasCancel && !bHasCancel) return 1;
    if (!aHasCancel && bHasCancel) return -1;
    return 0;
  });
  ...
  const getStatusColor = (status) => {
    switch (status.toLowerCase()) {
      ...
      case 'waiting': return 'text-gray-500 bg-gray-50 ring-1 ring-gray-100';
      case 'cancel': return 'text-red-500 bg-red-50 ring-1 ring-red-100';
      case 'overdue': return 'text-amber-500 bg-amber-50 ring-1 ring-amber-100';
  ...
  const isOverdue = (maximumIntervalDate) => {
  ...
  const sortOptions = [
    ...
    { id: 'cancel', label: 'Cancelled', icon: <XCircle className="w-4 h-4" /> },
  ];
