      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-gray-100">
            ...
            <th className="p-2 text-left text-sm font-semibold text-gray-600">Action</th>
          </tr>
        </thead>
        <tbody>
          {children && children.length > 0 ? (
            children.map((child) => {
              const isTracking = (child.status || "").toLowerCase() === "tracking"; // Kiểm tra status
                ...
                <tr key={child.id} className="border-b hover:bg-gray-50">
                    ...
                    <span
                      className={`inline-block px-2 py-1 text-sm font-medium rounded-full ${
                        (child.status || "active").toLowerCase() === "active"
                          ...
                          ? "bg-yellow-100 text-yellow-800" // Thêm màu cho Tracking
                  ...
                  <td className="p-2 text-sm text-gray-600">
                    {!isTracking ? (
                      ...
                      <span className="text-gray-400 italic">Actions disabled (Tracking)</span>
                    )}
                  </td>
                </tr>
              );
