...
const ModalUpdateStatus = ({ newStatus, setNewStatus, isStatusModalOpen, setIsStatusModalOpen, selectedRecord, childData, handleStatusUpdate }) => {
  ...
  // Kiểm tra trạng thái "Overdue" cho selectedRecord
  const overdue = selectedRecord ? isOverdue(selectedRecord.maximumIntervalDate, selectedRecord.status) : false;
      ...
      <DialogContent className="sm:max-w-[500px] border-blue-100">
        ...
        {selectedRecord && (
          <div className="py-4">
            ...
            {/* Nếu mũi tiêm bị "Overdue", chỉ hiển thị tùy chọn "Cancel" */}
            {overdue ? (
              <>
                <div className="text-sm mb-3 flex items-center text-blue-700">
                  <AlertTriangle className="h-4 w-4 mr-2" />
                  Current status: <span className="font-medium ml-1">Overdue</span>
                </div>
              ...
              <>
                {/* Hiển thị tùy chọn trạng thái cho "Waiting" */}
                ...
                {/* Hiển thị tùy chọn trạng thái cho "Schedule" */}
