...
export default function BodyVariantsHomePage() {
  ...
  const [isLoading, setIsLoading] = useState(true); // Đổi setLoading thành setIsLoading cho nhất quán
  const [error, setError] = useState(null); // Đổi setErr thành setError cho chuẩn naming convention
    ...
    const fetchDataAsync = async () => {
      try {
        ...
        const [vaccineRes, comboRes] = await Promise.all([
          axios.get("https://localhost:7280/api/Vaccine/getAllVacines"), // Sửa chính tả "Vacines" -> "Vaccines"
          axios.get("https://localhost:7280/api/VaccineCombo/getVaccineCombo"),
        ]);
        ...
        if (error.response) {
          ...
          setError(
            "Không thể kết nối đến server. Vui lòng kiểm tra mạng hoặc backend."
          );
        } else {
          setError("Không thể tải danh sách vắc-xin. Vui lòng thử lại sau.");
        }
      } finally {
        setIsLoading(false);
      }
  ...
  // Cập nhật danh sách sortVaccines khi vaccines hoặc combos thay đổi
  useEffect(() => {
                ...
                {itemList.map((vaccine) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() =>
                          dispatch(vaccineAction.deleteVaccine(vaccine.id))
                        }
